...
[`zulip-embed-react`](../packages/react/)). This page is the canonical
list — one entry per event, with the detail shape, trigger, and a
minimum-viable handler snippet.

Pair with [`ARCHITECTURE.md`](./ARCHITECTURE.md) for the event
pipeline that drives these from the transport layer,
[`TROUBLESHOOTING.md`](./TROUBLESHOOTING.md) for `zulip-error` codes,
and [`agents.md`](./agents.md) for the `zulip-confirmation-response`
...
rate-limited us. See [`TROUBLESHOOTING.md`](./TROUBLESHOOTING.md) for
...
See [`agents.md`](./agents.md#example--dm-with-tool-call--confirmation)
...
layer. See
[`TROUBLESHOOTING.md#messages-send-twice--duplicate-on-reconnect`](./TROUBLESHOOTING.md#messages-send-twice--duplicate-on-reconnect).
...
- [`docs/TROUBLESHOOTING.md`](./TROUBLESHOOTING.md) — `zulip-error`
  code-by-code handling guide, plus CSP + styling gotchas.
- [`docs/agents.md`](./agents.md) — `zulip-confirmation-response`
  in a full tool-call orchestration loop.
- [`docs/ARCHITECTURE.md`](./ARCHITECTURE.md) — how transport events
  become `ZulipClient` state become host `CustomEvent`s.
- [`packages/react/README.md`](../packages/react/README.md) — every
  `on*` prop on every React wrapper.
