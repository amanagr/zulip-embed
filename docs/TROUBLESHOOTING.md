...
| ------------------------ | ------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `unauthorized`           | The `email + api-key` pair is rejected, or the JWT-derived key isn't valid for the realm.  | Re-copy the bot's key from Zulip's bot-details page. JWT users: confirm `JWT_AUTH_KEYS[realm].key` matches what your backend signed with.                |
...
| `jwt-not-configured`     | You passed `auth-token` but the Zulip server doesn't have `JWT_AUTH_KEYS` set.             | Have an admin follow [`docs/jwt.md`](./jwt.md) on the server side. Cloud tenants: file a support ticket requesting JWT auth.                             |
...
2. **CSS custom properties on the host.** The element reads a small,
   ...
   want to brand:
    ...
    See [`ARCHITECTURE.md §Styling`](./ARCHITECTURE.md) for the full
    token list.
...
[`ARCHITECTURE.md §Agent-reply`](./ARCHITECTURE.md).
