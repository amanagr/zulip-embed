...
Pair this page with [`ARCHITECTURE.md`](./ARCHITECTURE.md) for the
transport / scope / event-pipeline model; with
[`events.md`](./events.md) for the full `CustomEvent` surface the
widgets dispatch; and with [`jwt.md`](./jwt.md) for the auth path the
...
[`theming.md`](./theming.md) for how to retint the whole palette
...
host (see [`events.md`](./events.md)). `payloadSig` is produced by
...
- [`docs/events.md`](./events.md) — the full `CustomEvent` surface,
  including `zulip-confirmation-response` detail shape.
- [`docs/ARCHITECTURE.md`](./ARCHITECTURE.md) — transport / scope /
  event pipeline at the architectural level.
- [`docs/theming.md`](./theming.md) — tool-card and confirmation-card
  CSS token overrides.
- [`docs/TROUBLESHOOTING.md`](./TROUBLESHOOTING.md) — error codes for
  when the provisional send fails (`rate-limited`, `unauthorized`).
- [`packages/react/README.md`](../packages/react/README.md) — React
